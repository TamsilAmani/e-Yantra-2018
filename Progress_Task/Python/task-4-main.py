...
rightThreshold = 300  # rightThreshold: Threshold value of the right white line sensor below which it is white and above which the detected surface is taken as black
midThreshold = 300  # midThreshold: Threshold value of the middle white line sensor below which it is white and above which the detected surface is taken as black
leftThreshold = 250  # leftThreshold: Threshold value of the left white line sensor below which it is white and above which the detected surface is taken as black

max_right = 620  # max_right : approx. maximum value given by the right photo-receptor of White Line Sensor
max_left = 620  # max_left: approx. maximum value given by the left photo-receptor of White Line Sensor

pid = 4  # pid: amount of variation assigned to dutyCycleRight and dutyCycleLeft

dutyCycleLeft = 20  # dutyCycleLeft: assigns duty cycle to left DC motor
dutyCycleRight = 20  # dutyCycleRight: assigns duty cycle to right DC motor
...
robotHeading = up  # robotHeading: indicates the direction of robot
robotLocation = 3  # robotLocation: tells us at which node robot is currently

number_of_nodes = 31  # number_of_nodes: total number of nodes on the arena
graph = [[' ' for c in range(number_of_nodes)] for r in range(number_of_nodes)]  # graph: contains the paths between the nodes
...
serviceNodeDictionary = {  # serviceNodeDictionary: assigns the node numbers of the services to their respective Ant-Hills
    0: [23, 25],
    1: [26, 28],
    2: [17, 19],
    3: [14, 16]
...
box_nodes = [0, 1, 2, 4, 5, 6, 14, 16, 17, 19, 23, 25, 26, 28]  # Contains list of nodes for which path last path traversal is not taken
...
def detect_marker(name):
    ...
    with open('eYRC#AB#2121.csv', 'a') as file:
...
def soft_turn(temp_cycle_left, temp_cycle_right):
    GPIO.output(Motor1A, GPIO.LOW)
    GPIO.output(Motor1B, GPIO.HIGH)
...
'''
*Function Name: reverse_soft_turn
*Input: temp_cycle_left -> value used for duty cycle of left DC motor, 
        temp_cycle_right -> value used for duty cycle of right DC motor,
*Output: Turn the line follower in the left/right direction to keep it on a straight line
*Logic: The right and left motors are rotated with different speeds to change direction
*Example Call: reverse_soft_turn(40, 50)
'''


def reverse_soft_turn(temp_cycle_left, temp_cycle_right):
    GPIO.output(Motor1A, GPIO.HIGH)
    GPIO.output(Motor1B, GPIO.LOW)
    rightMotor.start(temp_cycle_right)
...
def get_sensor_values():
    t1 = str(serialInput.readline()).strip("b'").strip("\\n").strip("\\r")
    if len(t1) is not 3:
...
def move_straight(move_time=0.0):
    ...
    if move_time > 0:
        end_time = time.clock() + move_time
        while end_time - time.clock() > 0:
            right_value, mid_value, left_value = get_sensor_values()

            temp_cycle_left = dutyCycleLeft + (((leftThreshold - left_value) * pid) / (max_left - leftThreshold))
            temp_cycle_right = dutyCycleRight + (((rightThreshold - right_value) * pid) / (max_right - rightThreshold))
    ...
    while (right_value > rightThreshold and mid_value > midThreshold and left_value > leftThreshold) or \
            (right_value > 500 and mid_value > 500) or (left_value > 500 and mid_value > 500):
        right_value, mid_value, left_value = get_sensor_values()
    ...
    while True:
        ...
        if (right_value > 500 and mid_value > 500) or (left_value > 500 and mid_value > 500):
            leftMotor.start(0)
            rightMotor.start(0)
            break
            ...
            if right_value < left_value:
                right_diff = abs(mid_value - 400) * 0.1
                left_diff = 0
            else:
                right_diff = 0
                left_diff = abs(mid_value - 400) * 0.1

            right_diff = min(right_diff, 3)
            left_diff = min(left_diff, 3)
            temp_cycle_left = dutyCycleLeft + (((leftThreshold - left_value) * pid) / (max_left - leftThreshold)) + left_diff
            temp_cycle_right = dutyCycleRight + (((rightThreshold - right_value) * pid) / (max_right - rightThreshold)) + right_diff
...
def move_reverse():
    ...
    while right_value > 300 and mid_value > 300 and left_value > 300:
        right_value, mid_value, left_value = get_sensor_values()
        reverse()

    while True:
        right_value, mid_value, left_value = get_sensor_values()
        if (right_value > rightThreshold and mid_value > midThreshold) or (left_value > leftThreshold and mid_value > midThreshold):
            leftMotor.start(0)
            rightMotor.start(0)
            break

        elif right_value < rightThreshold and mid_value > midThreshold and left_value < leftThreshold:
            reverse()

        elif left_value > leftThreshold:
            temp_cycle_left = dutyCycleLeft + (((leftThreshold - left_value) * pid) / (max_left - leftThreshold))
            temp_cycle_right = dutyCycleRight + (((rightThreshold - right_value) * pid) / (max_right - rightThreshold))

            reverse_soft_turn(temp_cycle_left, temp_cycle_right)

        elif right_value > rightThreshold:
            temp_cycle_left = dutyCycleLeft + (((leftThreshold - left_value) * pid) / (max_left - leftThreshold))
            temp_cycle_right = dutyCycleRight + (((rightThreshold - right_value) * pid) / (max_right - rightThreshold))
...
def get_supply_location(required_color_code):
    ...
    for _ in shrub_visited:
        closest_shrub = find_closest_unvisited_shrub()
        command_robot(closest_shrub)
        shrub_visited[closest_shrub] = True
        detected_color_code = scan_image()
        if detected_color_code is required_color_code:
            return closest_shrub
        shrub_nodes[detected_color_code].append(closest_shrub)
...
def scan_image():
    ...
    capture_image('temp_image.jpeg')

    image = cv2.imread("temp_image.jpeg")

    rgby_lower = [[60, 40, 130], [90, 120, 8], [85, 69, 15], [43, 110, 100]]  # Red,Green, Blue, Yellow
    rgby_upper = [[120, 100, 190], [105, 140, 70], [124, 120, 100], [75, 132, 140]]
    ...
    for color in range(4):
        ...
        ret, thresh = cv2.threshold(gray, 8, 255, 0)
        ...
        if largest is not None and cv2.contourArea(largest) > 3000:
            color += 1
...
def sharp_move_right():
    right_value, mid_value, left_value = get_sensor_values()

    while right_value > rightThreshold and mid_value > midThreshold and left_value > leftThreshold:
        right_value, mid_value, left_value = get_sensor_values()
    ...
    GPIO.output(Motor2A, GPIO.LOW)
    GPIO.output(Motor2B, GPIO.HIGH)
    rightMotor.start(dutyCycleRight - 2)
    leftMotor.start(dutyCycleRight)
    ...
    robotHeading = direction[(direction.index(robotHeading) + 1 + len(direction)) % len(direction)]
...
def sharp_move_left():
    ...
    GPIO.output(Motor2B, GPIO.LOW)
    rightMotor.start(dutyCycleRight)
    leftMotor.start(dutyCycleRight - 2)
...
def command_robot(destination):
    ...
    for path_number in range(len(path)):
        ...
        if ((robotHeading is up and path_direction is down) or (robotHeading is down and path_direction is up) or
            (robotHeading is left and path_direction is right) or (robotHeading is right and path_direction is left)) and \
                (not lastCommandReverse):
            move_reverse()
            lastCommandReverse = True


        elif ((robotHeading is up and path_direction is left) or (robotHeading is left and path_direction is down) or \
              (robotHeading is down and path_direction is right) or (robotHeading is right and path_direction is up)) or \
                ((robotHeading is up and path_direction is down) or (robotHeading is down and path_direction is up) or \
                 (robotHeading is left and path_direction is right) or (robotHeading is right and path_direction is left)):
            if lastCommandReverse:
                sharp_move_left()
            sharp_move_left()
...
if __name__ == "__main__":
    ...
    with open('eYRC#AB#2121.csv', 'w') as file:
        writer = csv.writer(file, delimiter=',', quoting=csv.QUOTE_ALL)
        writer.writerow(['SIM', 'ID Detected'])

    # ---- Capturing ArUco Markers
    # Turn the robot continously for approximately 1.1 seconds to turn it by an angle of 45 degrees
    aruco_left_turn(1.1)
    capture_image('SIM0.jpg')
    detect_marker('SIM0.jpg')
    print("ID DETECTED:", list_of_id[0])
    # Turn the robot continously for approximately 2.5 seconds to turn it by an angle of 90 degrees
    aruco_left_turn(2.5)
    capture_image('SIM3.jpeg')
    detect_marker('SIM3.jpeg')
    print("ID DETECTED:", list_of_id[1])
    aruco_left_turn(2.5)
    capture_image('SIM2.jpeg')
    detect_marker('SIM2.jpeg')
    print("ID DETECTED:", list_of_id[2])
    aruco_left_turn(2.5)
    capture_image('SIM1.jpeg')
    detect_marker('SIM1.jpeg')
    print("ID DETECTED:", list_of_id[3])
    aruco_left_turn(1.1)
