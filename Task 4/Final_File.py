...
rightThreshold = 100  # rightThreshold: Threshold value of the right white line sensor below which it is white and above which the detected surface is taken as black
...
leftThreshold = 100  # leftThreshold: Threshold value of the left white line sensor below which it is white and above which the detected surface is taken as black
...
Kp = 30  # pid: amount of variation assigned to dutyCycleRight and dutyCycleLeft

dutyCycleLeft = 23  # dutyCycleLeft: assigns duty cycle to left DC motor
dutyCycleRight = 23  # dutyCycleRight: assigns duty cycle to right DC motor
...
def move_straight(move_time=0.0):
    ...
    while True:
        ...
        if (((right_value > 600 and mid_value > 600) or (left_value > 600 and mid_value > 600)) and (
                time_diff > 0.25)) or (0 < move_time <= time.clock() - node_start_time):
            lastNodeDetectedTime = time.clock()
            leftMotor.start(0)
            rightMotor.start(0)
            break
...
def move_reverse():
    ...
    while True:
        ...
        global lastNodeDetectedTime
        if (((right_value > 600 and mid_value > 600) or (left_value > 600 and mid_value > 600)) and (
                time.clock() - lastNodeDetectedTime > 0.3)):
            lastNodeDetectedTime = time.clock()
            leftMotor.start(0)
            rightMotor.start(0)
            break
...
def sharp_move_right(time_len=0.37, n=1):
    straight()
    sleep(time_len)
    GPIO.output(Motor1A, GPIO.HIGH)
    GPIO.output(Motor1B, GPIO.LOW)
...
def sharp_move_left(time_len=0.35, n=1):
    straight()
    sleep(time_len)
    GPIO.output(Motor1A, GPIO.LOW)
    GPIO.output(Motor1B, GPIO.HIGH)
    ...
    while left_value < leftThreshold:
        right_value, mid_value, left_value = get_sensor_values()
        continue
...
if __name__ == "__main__":
    ...
    # Turn the robot continously for approximately 2.5 seconds to turn it by an angle of 45 degrees
    aruco_left_turn(1)
